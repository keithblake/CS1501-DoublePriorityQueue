...
public class PricePriorityQueue {
	...
	public PricePriorityQueue(int size) {
		max = size;
		n = 0;
		arr = new Car[max];
	...
	public void updateCar(Car newCar, int index, int category) {
		...
		if (category == 1) {	//may need to rearrange queue
			swim(index, newCar);
			sink(index);
		}
	}
		...
		for (int i = 0; i < n; i++) {
			if (arr[i].getMake().equalsIgnoreCase(make) && arr[i].getModel().equalsIgnoreCase(model)) {
				if (index == -1)	//if first time
					index = i;
				else if (arr[i].getPrice() < arr[index].getPrice())
					index = i;
	...
	private void swim(int index, Car newCar) {
		...
		while (arr[parentIndex].getPrice() > 1 && arr[parentIndex].getPrice() > arr[currIndex].getPrice()) {
			//swap positions
	...
	private void sink(int index) {
		...
		while (2*index+2 < n && (arr[index].getPrice() > arr[2*index+1].getPrice() || 
				arr[index].getPrice() > arr[2*index+2].getPrice())) {
			if (arr[index].getPrice() > arr[2*index+1].getPrice()) {
				...
				index = 2*index+1;
			} else if (arr[index].getPrice() > arr[2*index+2].getPrice()) {
				//swap them, update index
				Car tmp = arr[index];
